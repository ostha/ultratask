                          ...
                          @foreach ($results as $res )
                            ...
                            <tr>
                               ...
                               <td class="text-center">
                                   @if($res->featimg != null && file_exists(public_path('')."/assets/media/products/".$res->featimg))
                                   <img class="rounded img-fluid avatar-40" src="{{ asset('assets/media/products/thumbnails') }}/{{ $res->featimg }}" alt="profile">
                                   @else
                               ...
                               <td>
                                  <div class="flex align-items-center list-user-action">
                                     <a class="iq-bg-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" href="{!! route('product.edit',$res->id) !!}"><i class="ri-pencil-line"></i></a>
                                     <a onclick="event.preventDefault();  document.getElementById('fif').submit();" class="iq-bg-primary" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" href="{{ url('/') }}"><i class="ri-delete-bin-line"></i>
                                        <form id = "fif" action="{!! route('product.destroy',$res->id) !!}" method="POST" style="display: none">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                                        </form>
